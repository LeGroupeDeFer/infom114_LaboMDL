...
fn add_new_tag() {
    ...
    assert_eq!(Tag::all(&conn).len(), 1);
    assert!(Tag::by_label(&conn, &tag).is_some());
}
...
fn delete_tag() {
    ...
    assert!(Tag::by_label(&conn, &tag).is_none());
}
...
fn update_tag() {
    ...
    let existing_tag = Tag::by_label(&conn, &existing_tag_label).unwrap();
    ...
    assert!(Tag::by_label(&conn, &existing_tag_label).is_none());

    // panics if the new label is not found
    let new_tag = Tag::by_label(&conn, &tag).unwrap();
...
fn update_tag_with_already_existing_label() {
    ...
    let existing_tag = Tag::by_label(&conn, &existing_tag_label).unwrap();
    let new_tag = Tag::by_label(&conn, &new_tag_label).unwrap();
    ...
    assert_eq!(
        Tag::by_label(&conn, &existing_tag_label).unwrap().id,
        existing_tag.id
    );
    assert_eq!(Tag::by_label(&conn, &new_tag_label).unwrap().id, new_tag.id);
}
...
fn update_tag_with_malformed_json() {
    ...
    assert!(Tag::by_label(&conn, &tag_label).is_some());
